...
class DeviceConnection(object):
        ...
        while not self.killed:
            ...
            try:
                ...
                while line.find('\r\n') > -1:
                    ...
                    self.timeout_handler_onlinecheck = ioloop.IOLoop.current().call_later(HEARTBEAT_PERIOD_SEC,
                                                                                          self._online_check)

                    if self.timeout_handler_offline:
                        ioloop.IOLoop.current().remove_timeout(self.timeout_handler_offline)
                    self.timeout_handler_offline = ioloop.IOLoop.current().call_later(
                        HEARTBEAT_PERIOD_SEC + HEARTBEAT_NEGATIVE_CHECK_DELAY_SEC, \
                    ...
                    try:
                        ...
                        if state:
                            ...
                            if self.sn in DeviceConnection.state_waiters and len(
                                    DeviceConnection.state_waiters[self.sn]) > 0:
    ...
    def _online_check(self):
        ...
        try:
            ...
            if self.timeout_handler_onlinecheck:
                ioloop.IOLoop.current().remove_timeout(self.timeout_handler_onlinecheck)
            self.timeout_handler_onlinecheck = ioloop.IOLoop.current().call_later(HEARTBEAT_FAST_PING_DELAY_SEC,
                                                                                  self._online_check)
        except iostream.StreamClosedError:
            gen_log.error(
    ...
    def kill_junk(self):
        gen_log.debug("kill_junk: %s" % self)
        self.killedjunk = True
        self.kill_myself()
    ...
    def on_close(self):
        ...
        # objgraph.show_backrefs(self.on_close, max_depth=10, filename = "direct.png")
        print "_gcleak ref count1 :%d" % (sys.getrefcount(self))
...
class DeviceServer(TCPServer):
    ...
    def handle_stream(self, stream, address):
        ...
        self.aaa = self.aaa + 1
        gen_log.debug("connect_request_count %s", str(self.aaa))

        # stream.close()
        # stream.set_close_callback(None)
        # del conn

        # objgraph.show_backrefs(conn, max_depth=10, filename="direct.png")
    ...
    def __init__(self, db_conn, cursor):
        handlers = [
            ...
            (r"/v1/node/(?=function)(.+)", NodeFunctionHandler,
             dict(conns=DeviceServer.accepted_xchange_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
            (r"/v1/node/(?=setting)(.+)", NodeSettingHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
            (r"/v1/node/event[/]?", NodeEventHandler, dict(conns=DeviceServer.accepted_xchange_conns)),
            (r"/v1/node/config[/]?", NodeGetConfigHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
            (r"/v1/node/resources[/]?", NodeGetResourcesHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
            (r"/v1/ota/trigger[/]?", FirmwareBuildingHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
            (r"/v1/ota/status[/]?", OTAStatusReportingHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
            (r"/v1/cotf/(project)[/]?", COTFHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened)),
        ]
...
class myApplication_OTA(web.Application):
    def __init__(self, db_conn, cursor):
        handlers = [
            (r"/v1[/]?", IndexHandler),
            (r"/v1/test[/]?", TestHandler),
            (r"/v1/ota/bin", OTAFirmwareSendingHandler,
             dict(conns=DeviceServer.accepted_ota_conns, state_waiters=DeviceConnection.state_waiters,
                  state_happened=DeviceConnection.state_happened))
        ]
        self.conn = db_conn
...
class Statistics(object):
    ...
    def report_statistics(self):
        ...
        gen_log.info('gc threshold    : {}'.format(gc.get_threshold()))
