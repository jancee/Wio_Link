...
def gen_wrapper_registration(instance_name, info, arg_list):
    ...
    for fun in info['Reads'].items():
        fp_wrapper_h.write(
            'bool __%s_%s(void *class_ptr, char *method_name, void *input_pack);\r\n' % (grove_name, fun[0]))
        ...
        str_reg_method += '    rpc_server_register_method("%s", "%s", METHOD_READ, %s, %s, arg_types);\r\n' % \
                          (instance_name, fun[0].replace('read_', ''), '__' + grove_name + '_' + fun[0],
                           instance_name + "_ins")
    ...
    for fun in info['Writes'].items():
        ...
        str_reg_method += '    rpc_server_register_method("%s", "%s", METHOD_WRITE, %s, %s, arg_types);\r\n' % \
                          (instance_name, fun[0].replace('write_', ''), '__' + grove_name + '_' + fun[0],
                           instance_name + "_ins")
    ...
    if info['HasPowerOnFunc']:
        fp_wrapper_h.write(
            'bool __%s_on_power_on(void *class_ptr, char *method_name, void *input_pack);\r\n' % grove_name)
    ...
    if info['HasPowerOffFunc']:
        fp_wrapper_h.write(
            'bool __%s_on_power_off(void *class_ptr, char *method_name, void *input_pack);\r\n' % grove_name)
...
def gen_and_build(build_phase, app_num, user_id, node_sn, node_name, server_ip, json_drivers, json_connections):
    ...
    if config:
        for grove_instance_name in config.keys():
            ...
            if grove:
                ret, inc, dec, method, wellknown = gen_wrapper_registration(grove_instance_name, grove,
                                                                            config[grove_instance_name][
                                                                                'construct_arg_list'])
                if (ret == False):
                    return False
